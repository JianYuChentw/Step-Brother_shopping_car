...
async function update_product_stock(product_id, size_id, quantity) {
  try {
    ...
    const [stock_results] = await connection.query(get_stock_query, [
      product_id,
      size_id,
    ]);
    ...
    await connection.query(update_stock_query, [
      updated_stock,
      product_id,
      size_id,
    ]);
    ...
    console.log(
      `商品 ID ${product_id} 尺寸 ${size_id} 的庫存數量已更新為 ${updated_stock}`
    );
    return true;
  } catch (error) {
