...
async function update_product_name(product_id, newProductName) {
  ...
  `;
    ...
    if (update_result.affectedRows > 0) {
      console.log(
        `商品編號 ${product_id} 的商品名稱已更新為 "${newProductName}"`
      );
      return true;
    } else {
...
// 取得單筆商品單尺寸庫存量
async function get_single_product_single_size_stock_quantity(
  product_id,
  size_id
) {
  try {
...
async function create_product_stock_quantity(product_id, size_id, quantity) {
  try {
    ...
    console.log(
      `商品 ID ${product_id} 尺寸 ${size_id} 的庫存數量已更新為 ${quantity}`
    );
...
// 商品價格、狀態及庫存異動
async function update_product_status_and_stock(
  product_id,
  product_status,
  size_stock,
  price
...
// 上傳產品
async function create_detail_and_product_data(
  brand_id,
  product_name,
  price,
  class_id
  ...
  try {
    ...
    const existing_detail = await use_product_name_get_detail_data(
      product_name
    );
    ...
    const product_data_query =
      'INSERT INTO product_data (brand_id, detail_id, class_id) VALUES (?, ?, ?)';
    const product_data_values = [brand_id, detail_id, class_id];
    const [product_data_result] = await conn.query(
      product_data_query,
      product_data_values
    );
    ...
    const stock_query =
      'INSERT INTO product_stock (product_data_id, size_id) VALUES (?, ?)';
...
async function return_quantity(product_id, size_id, quantity) {
  try {
    ...
    // 執行查詢，獲取當前庫存數量
    const [stockResults] = await connection.query(currentStockQuery, [
      product_id,
      size_id,
    ]);
    const currentQuantity =
      stockResults.length > 0 ? stockResults[0].stock_quantity : 0;
    ...
    console.log(
      `商品 ID ${product_id} 尺寸 ${size_id} 的庫存數量已更新為 ${newQuantity}`
    );
...
async function return_stock(product_name, size_id, quantity) {
  try {
    ...
    // 執行查詢，並獲取商品的ID
    const [product_id] = await connection.execute(query_product_id, [
      product_name,
    ]);
